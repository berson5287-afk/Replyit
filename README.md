...
polish is off, the endpoint is down, or a draft is rejected.

Greetings are generated from the clock, in two halves split at noon:
**"Good morning"** before 12:00, **"Good afternoon"** from 12:00 on. There is
no "Good evening" — across 30 greetings in the confirmed corpus he writes
"Good morning" 14 times and "Good afternoon" 12, and "Good evening" exactly
never. A reply drafted at 6pm is read the next morning anyway, and a greeting
he would not use is a tell that the mail was not written by him.

A role address (`estimating@`, `sales@`) gets no name rather than
"Good morning Estimating,".

Neither half of the greeting is the model's to choose, so the whole line is
forced back to the template's before a polished draft ships — the name because
it was resolved from the actual sender, the time of day because it was resolved
from the actual clock. Repaired rather than rejected, since the rest of the
draft is usually fine.
